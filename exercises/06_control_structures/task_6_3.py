...
for intf in trunk.keys():
	...
	for comms in trunk_template:
		if comms.endswith("allowed vlan"):
			if trunk[intf][0] == 'add':
				print(comms + ' add ' + ','.join(trunk[intf][1:]))
			elif trunk[intf][0] == 'only':
				print(comms + ' ' +','.join(trunk[intf][1:]))
			else:
				print(comms + ' remove ' + ','.join(trunk[intf][1:]))
		else:
			print(comms)
